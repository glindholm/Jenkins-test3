...
public abstract class BaseDAO<PERSISTENTCLASS, IDTYPE extends Serializable>
{
	...
	/**
	 * Build up a <code>Criteria</code> object using the specified
	 * <code>Criterion</code>s.
	 *
	 * @param daoContext
	 *            {@link DataAccessContext} object for the database.
	 * @param criterions
	 ...
	 *         database.
	 */
	public Criteria buildCriteria(DataAccessContext daoContext,
	...
	public PERSISTENTCLASS getByCriteria(DataAccessContext daoContext,
			String comment, Criterion... criterions)
	{
	...
	/**
	 * Insert a new entity into the database. This method recognizes objects
	 * that implement the {@link CreatedDate} and {@link UpdatedDate}
	 * interfaces, and will set the current <code>Date</code> into them before
	 * inserting them.
	 *
	 * @param daoContext
	 *            {@link DataAccessContext} object for the database.
	 ...
	 */
	public PERSISTENTCLASS insert(DataAccessContext daoContext,
	 ...
	 * "saveOrUpdate", so it can also be used to insert a new entity. This
	 * method recognizes objects that implement the {@link CreatedDate} and
	 * {@link UpdatedDate} interfaces, and will set the current
	 * <code>Date</code> into them, as appropriate, before inserting or updating
	 * them.
	 *
	 * @param daoContext
	 *            {@link DataAccessContext} object for the database.
	 ...
	 */
	public PERSISTENTCLASS update(DataAccessContext daoContext,
			PERSISTENTCLASS entity)
	{
	 ...
	 * directly deleting a persistent instance as in
	 * {@link #delete(DataAccessContext, Object)}).
	 * <p>
	 ...
	 *
	 * <p>
	 * This is efficient, since it does not require the object to have been
	 * fetched from the database in order to delete it. However, this method
	 * will fail to operate properly if the object in question has an
	 * &x40;ElementCollection, since those do not cascade properly under bulk
	 * deletes, or if it is not set up to cascade deletes to any other objects
	 * that may have foreign keys to it.
	 * </p>
	 * </li>
	 * <li>
	 * This method assumes that the unique ID for the object is, in fact, called
	 * <code>id</code>.</li>
	 * </ol>
	 *
	 * @param daoContext
	 ...
	 * @return <code>true</code> if an object was actually deleted,
	 *         <code>false</code> if there was no matching object in the
	 *         database.
	 ...
	 * "bulk delete."
	 * <p>
	 * While efficient, this method doesn't cascade, so if there are other
	 * objects in the database that maintain a foreign key relationship into
	 * this table, this operation will fail with a constraint violation
	 * exception unless those objects are deleted first. Thus, in this case,
	 * there are two options:
	 * </p>
	 * <ul>
	 * <li>Use <code>deleteAll</code> instead of this method</li>
	 * <li>Override this method in a derived DAO and clear the dependent tables
	 * before delegating upward to this method.</li>
	 *
	 * @param daoContext
	 *            {@link DataAccessContext} object for the database.
	 ...
	 * @return <code>List</code> of all the entities. The returned
	 *         <code>List</code> will not be <code>null</code>, but may be
	 *         empty.
	 */
	 ...
	 *         empty.
	 */
	public List<PERSISTENTCLASS> listAllDistinct(DataAccessContext daoContext,
			Order order)
	{
		Criteria criteria = buildCriteria(daoContext).setComment(".listAll()");
		if (order != null)
	...
	public List<PERSISTENTCLASS> listByCriteria(DataAccessContext daoContext,
			String comment, Order order, Criterion... criterions)
	{
		Criteria criteria = buildCriteria(daoContext, criterions);
	...
	/**
	 * Return the <code>List</code> of objects that satisfy the specified
	 * <code>Criteria</code>. (Utility method to handle type safety).
	 *
	 * @param criteria
	...
	/**
	 * Return the <code>List</code> of objects that satisfy the specified
	 * <code>Criteria</code>. (Utility method to handle type safety). This
	 * method does two things: </p>
	 * <ol>
	 * <li>It runs the <code>Criteria</code> through the
	 * {@link #setDistinctRoot(Criteria)} utility method to help handle issues
	 * associated with multiple results being returned when the class in
	 * question has one or more <code>&#x40;EmbeddedCollection</code>s.</li>
	 * <li>It casts the returned object to the appropriate
	 * <code>PERSISTENTCLASS</code>, thus eliminating the need to do the cast in
    ...
    /**
     * Return the list of object ids that satisfy the specified
     * <code>Query</code>. (Utility method to handle type safety).
     *
     * @param query <code>Query</code> object specifying retrieval of
     *        <code>IDTYPE</code>.
	...
	/**
	 * Utility method to return the unique entity that satisfies the specified
	 * <code>Criteria</code> specification.
	 * <p>
     ...
     * This will create either an "IS NULL" or an "EQ" Criterion based on
     * <code>value</code>. If <code>value</code> is <code>null</code> then
     * <code>Restrictions.isNull(propertyName)</code> will be returned,
