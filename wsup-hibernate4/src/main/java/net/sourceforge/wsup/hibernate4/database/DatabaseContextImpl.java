...
{
    private static final Logger log = LoggerFactory.getLogger(DatabaseContextImpl.class);
    ...
    {
        beginSession(false);
        return session.getLobHelper().createClob(string);
    }
    ...
    {
        beginSession(false);
        return session.getLobHelper().createClob(reader, length);
    }

    /**
     * Get the underlying Hibernate <code>Session</code> object.
     * 
     * @return <code>Session</code> object if a session is open, <code>null</code> otherwise.
     */
     ...
     * session has actually been started. If necessary, calls <code>beginSession(false)</code>;
     * 
     ...
     * 
     * @return <code>Transaction</code> object if a transaction is open, <code>null</code>
     *         otherwise.
     */
